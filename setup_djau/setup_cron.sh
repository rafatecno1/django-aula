...
echo -e "${NEGRITA}La aplicación DJANGO-AULA habrá quedado instalada con éxito.${RESET}"
echo -e "\n"
echo "A partir de ahora DJANGO-AULA (DjAu) ya está listo para recibir los datos de su centro educativo."
echo -e "\n"
echo -e "${NEGRITA}➡️ SIGUIENTE PASO: Carga de Datos y Configuración del curso escolar${RESET}"
echo -e "\n"
echo -e "   Consulte las instrucciones detalladas para todo el proceso de la carga de datos en la Wiki del proyecto:"
...
echo -e "${C_TITULO}================================================${RESET}"
echo "L'aplicatiu DJNGO-AULA s'ha instal·lat amb ${C_ERROR}DEBUG = True${RESET}, visible a l'inici de l'arxiu /aula/settings_local.py."
echo "Aquest mode és útil abans de la càrrega de dades pel curs escolar ${C_INFO}(configuració inicial del curs)${RESET} i per ${C_INFO}depuració d'errors${RESET} quan l'aplicatiu està funcionant a mig curs."
echo ""
echo "En mode DEBUG:"
...
echo "----------------------------------------"
echo "1. ${C_EXITO}Un cop finalitzada la càrrega inicial de dades i configurat els usuaris.${RESET}"
echo "2. Edita el fitxer ${C_EXITO}'$FULL_PATH/aula/settings_local.py'${RESET}"
echo "3. Canvia la línia:"
echo "   ${C_ERROR}DEBUG = True${RESET}"
echo "   A:"
echo "   ${C_EXITO}DEBUG = False${RESET}"
echo "4. ${C_EXITO}Reinicia el servidor Apache (sudo systemctl restart apache2) o, per a més seguretat, tot el servidor.${RESET}"
echo -e "\n"
echo -e "${C_TITULO}====================================================================================================${RESET}"
echo -e "\n"
